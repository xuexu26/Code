...
write.csv(combat_data,"file name.csv")
...
library(tidyverse)
library(limma)
exp<-read.csv("file name.csv",row.names = 1)

fen<-read.csv("file name.csv",row.names = 1)
...
print(top20_simple)
write.csv(DEG_sorted, "sorted_DEGs.csv")
...
library(pheatmap)
library(dplyr)
diff_gene <- read.csv("diff-anlysis resulit.csv",row.names = 1)
...
KEGG=read.table("KEGG.txt", header=T, sep="\t", check.names=F)
KEGG = KEGG[c(1:10),]
colnames(KEGG)
...
p1 <- ggplot() +
  ...
  mytheme
p1
ggsave("sankey_dot_plot.pdf", p1, width = 5, height = 7) 
...
p2
ggsave("sankey_dot_plot2.pdf", p2, width = 10, height = 7) 
p3 <- ggdraw() + draw_plot(p2) + draw_plot(p1, scale = 0.5, x = 0.50, y=-0.17, width=0.68, height=1.3)
p3
ggsave("sankey_dot_plot3.pdf", p3, width = 10, height = 7) 
...
write.csv(geneCoef,"Core Gene Coefficient.csv")
...
genelist<-read.csv("Classification.csv",row.names = 1)
group <- as.character(genelist$type)
rf=randomForest(as.factor(group)~., data=data, ntree=500)
pdf(file="Random Forest Plot.pdf", width=6, height=6)
...
p1
pdf(file="important gene1.pdf", width=6, height=6)
print(p1)
dev.off()
...
p3
pdf(file="important gene12.pdf", width=6, height=6)
print(p3)
dev.off()

rfGenes=importance[order(importance[,"importance"], decreasing = TRUE),]
write.csv(rfGenes,"Gene Feature.csv")
...
library(pROC)
library(ggsci)
rt=read.csv("输入文件.csv",row.names = 1)
...
write.table(outTab, file="Nomoscore.txt", sep="\t", quote=F, col.names=F)
...
library(pROC)   
rt=read.csv("file name(Irf1) .csv",row.names = 1)
...
pdf("ROC(Irf1 gene).pdf",width=5,height=5)
...
library(pROC)   
rt=read.csv("file name(Tnfaip3) .csv",row.names = 1)
...
pdf("ROC(Tnfaip3 gene).pdf",width=5,height=5)
...
rt=read.csv("file name(Irf1 and Tnfaip3).csv",row.names = 1)  
y=colnames(rt)[1]
...
pdf("ROC(Multiple genes).pdf",width=5,height=5)
...
ssgsea_data <- read.csv("ssgsea.1.csv", row.names = 1, check.names = FALSE)
...
exprSet <- read.csv("exprSet.csv", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
